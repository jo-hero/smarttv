 ...
 * @author	Sonal Bansal
 */
public class JOCookieParser implements CookieParser
{
	...
	public boolean sendCookieWithURL(Cookie c,URL url,boolean bRespectExpires)
	{
		...
		if("0".equals(c.getVersion())) {
			return(domainMatch(url,domain) && pathMatch(url,path) && (bRespectExpires ? !c.hasExpired() : true));
		} else {
		...
		*/
		...
		while(it.hasNext())
		{
			...
			if(!Utils.matchQuotes(value))
			{
				if(bStrict)
				{
					throw new MalformedCookieException("Unmatched quotes throughout header.","SBCL_0009",JOCookieParser.class,"parseSetCookieV1");
				}
			}
	...
	public static final CookieJar parseSetCookieV0(Header responseHeader, URL url,boolean bStrict) throws MalformedCookieException
	{
		...
		while(it.hasNext())
		{
			...
			if(!Utils.matchQuotes(value))
			{
				if(bStrict)
				{
					throw new MalformedCookieException("Unmatched quotes throughout header.","SBCL_0009",JOCookieParser.class,"parseSetCookieV0");
				}
			}
	...
	public static Cookie parseSingleCookieV0(String s, URL url, boolean bStrict) throws MalformedCookieException
	{
		...
		*/
		...
		if(!Utils.matchQuotes(s))
		{
			if(bStrict)
			{
				throw new MalformedCookieException("Unmatched quotes in cookie.","SBCL_0010",JOCookieParser.class,"parseSingleCookieV0");
			}
		}
		...
		while(st.hasMoreTokens())
		{
			...
			if(Utils.isNullOrWhiteSpace(attr))
			{
				if(bStrict)
				{
					throw new MalformedCookieException("Wierd cookie.","SBCL_0002",JOCookieParser.class,"parseSingleCookieV0");
				}
			...
			else if("expires".equalsIgnoreCase(attr))
			{
				...
				if(d == null && bStrict)
				{
					throw new MalformedCookieException("Unparseable expires.","SBCL_0011",JOCookieParser.class,"parseSingleCookieV0");
				}
		...
		if(!c.isValid())
		{
			if(bStrict)
			{
				throw new MalformedCookieException("Invalid cookie.","SBCL_0012",JOCookieParser.class,"parseSingleCookieV0");
			}
		}
	...
	public static Cookie parseSingleCookieV1(String s, URL url, boolean bStrict) throws MalformedCookieException
	{
		...
		*/
		...
		if(!Utils.matchQuotes(s))
		{
			if(bStrict)
			{
				throw new MalformedCookieException("Unmatched quotes in cookie.","SBCL_0010",JOCookieParser.class,"parseSingleCookieV1");
			}
		}
		...
		while(st.hasMoreTokens())
		{
			...
			{/*
				...
				{
					throw new MalformedCookieException("Non-conforming cookie.","SBCL_0001",JOCookieParser.class,"parseSingleCookieV1");
				}*/
			...
			if(Utils.isNullOrWhiteSpace(c.getName()))
			{
				if(attr.startsWith("$"))
				{
					throw new MalformedCookieException("Non-conforming cookie.","SBCL_0003",JOCookieParser.class,"parseSingleCookieV1");
				}
			...
			else if("commenturl".equalsIgnoreCase(attr))
			{
				...
				catch(MalformedURLException mue)
				{
					if(bStrict)
					{
						throw new MalformedCookieException("Invalid data in Cookie.", mue, "SBCL_0004",JOCookieParser.class,"parseSingleCookieV1");
					}
				}
			}
			...
			else if("max-age".equalsIgnoreCase(attr))
			{
				...
				catch(NumberFormatException nfe)
				{
					if(bStrict)
					{
						throw new MalformedCookieException("Invalid data in Cookie.", nfe, "SBCL_0005",JOCookieParser.class,"parseSingleCookieV1");
					}
				}
			}
			else if("path".equalsIgnoreCase(attr))
			...
			else if("port".equalsIgnoreCase(attr))
			{
				...
				catch(NumberFormatException nfe)
				{
					if(bStrict)
					{
						throw new MalformedCookieException("Invalid data in Cookie.", nfe, "SBCL_0006",JOCookieParser.class,"parseSingleCookieV1");
					}
				}
			}
		...
		if(!c.isValid())
		{
			if(bStrict)
			{
				throw new MalformedCookieException("Invalid cookie.","SBCL_0012",JOCookieParser.class,"parseSingleCookieV1");
			}
		}
		else if(!allowedCookie(c,url,bStrict))
		{
	...
	public static boolean isRFC2965CookieString(String c) throws MalformedCookieException
	{
		...
		*/
		...
		if(s == null)
		{
			throw new MalformedCookieException("Non-conforming Cookie.","SBCL_0008",JOCookieParser.class,"isRFC2965CookieString");
		}
