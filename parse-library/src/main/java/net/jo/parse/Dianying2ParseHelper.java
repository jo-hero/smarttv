...
public class Dianying2ParseHelper implements IParseHelper {
    ...
    public Dianying2ParseHelper(JSONObject config, NavItem nav_template){
        this.config = config;
    ...
    public LinkedHashMap<String, String> parseHomePage(ArrayList<VideoGroup> vgs) {
        ...
        for(int i=0;i<modules.size();i++){
            ...
            String module_title = modules.get(i).getElementsByClass("module-title").get(0).ownText().trim();
    ...
    public ParseResult parse(String episode_tag, String video_name, String video_alias_name, String resource_tag, String resource_url, Callback callback) throws Exception {
        ...
        callback.call(40, resource_url);
        VideoGroup vg = this.parse(result.getResult(), "guess_list", 10);
        Constants.ParseCallback.call(vg);
        try {
            Document doc = Jsoup.parse(result.getResult());
            Element player_box_main = doc.getElementsByClass("player-box-main").get(0);
            ...
            if (!Utils.isEmpty(player_url)) {
                if(player_data.getString("from").contains("m3u8") || player_url.endsWith(".m3u8") || player_url.endsWith(".mp4")){
                    ...
                    return new ParseResult(URLDecoder.decode(player_url,"UTF-8"), result.getFinal_URL(),null);
                } else {
                    callback.call(50, resource_url);
                    player_url = this.serverUrl + "player/?url="+player_url;
                    result = null;//flag -> play
                    while (result == null || !result.isSign()) {
                        result = this.hsu.doGetBody(player_url, resource_url, this.getHeaders(), "UTF-8");
                        if(result.indexOf(true,"title>检测中","location.href")){
                            result = this.hsu.doGetBody(this.getLocation(result), resource_url, this.getHeaders(), "UTF-8");
                        }
                    }
                    player_url = Utils.getString(result.getResult(),"url\":\\s*\"(.*?)\",\\s*");
                    if(!Utils.isEmpty(player_url)){
                        ...
                        callback.call(60, "/jx/?url="+player_url);
    ...
    public VideoGroup doSearch(String keyword) {
        ...
        while (result == null || !result.isSign()) {
            try {
                result = this.hsu.doGetBody(this.getServerUrl()+"search-" + URLEncoder.encode(keyword, "UTF-8") + "-------------/", this.getServerUrl(), null, "UTF-8");
            } catch (Exception e) {}
        }
        vg.setName("search");
        ArrayList<VideoInfo> arrayList = new ArrayList<>();
        Elements searchList = Jsoup.parse(result.getResult()).getElementById("main").getElementsByClass("module-search-item");
        if (!Utils.isEmpty(searchList)) {
            for (int i = 0; i < searchList.size(); i++) {
                VideoInfo videoInfo = new VideoInfo();
                Element element = searchList.get(i);
                videoInfo.setName(element.getElementsByClass("video-info-header").get(0).getElementsByTag("h3").text());
                videoInfo.setUrl(Utils.resolveURI(this.getServerUrl(), element.getElementsByTag("a").get(0).attr("href")));
                videoInfo.setImage(Utils.resolveURI(result.getFinal_URL(), element.getElementsByTag("img").attr("data-src")));
                videoInfo.setTag(element.getElementsByClass("video-serial").text().trim());
                videoInfo.setDescription("");
